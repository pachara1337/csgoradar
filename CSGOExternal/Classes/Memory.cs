...
public class Memory
{
    public static Process Target = default;
    ...
    public static Enums.InitalizeResult Initalize(string process_name)
    {
        return Initalize(process_name, default);
    }

    public static Enums.InitalizeResult Initalize(string process_name, Enums.ProcessAccessFlags extra_flags = default)
    {
        ...
        if (processes.Length != 0)
        {
            try
            {
                Target = processes.FirstOrDefault();
                ProcessName = Target.ProcessName;
                Id = Target.Id;
                BaseAddress = (Int32)Target.MainModule.BaseAddress;
                Handle = WinAPI.OpenProcess((uint)(Enums.ProcessAccessFlags.VirtualMemoryOperation | Enums.ProcessAccessFlags.VirtualMemoryRead | Enums.ProcessAccessFlags.VirtualMemoryWrite | extra_flags), false, Id);
                return Enums.InitalizeResult.OK;
            }
            catch { return Enums.InitalizeResult.ACCES_VIOLATION_ERROR; }
        }
        else
    ...
    public static void RestoreDefaults()
    {
        ...
        BaseAddress = default;
        if (Handle == default)
            WinAPI.CloseHandle(Handle); Handle = default;
    }
    ...
    public static int ScanPattern(string module_name, string signature)
    {
        ...
        int ModuleAddress = -1;
        foreach (ProcessModule process_module in Target.Modules)
        {
    ...
    {
        [DllImport("kernel32.dll", SetLastError = true)] // https://www.pinvoke.net/default.aspx/kernel32.openprocess
        ...
        [DllImport("kernel32.dll", SetLastError = true)] // https://www.pinvoke.net/default.aspx/kernel32.closehandle
